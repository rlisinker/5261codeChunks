...
Before running the code in this document, please:
  ...
  2. Make sure you have installed the packages `rmarkdown`, `mosaic`, and `readr`.
  3. Download **statescountries.csv** and **ExampleData.csv** and put them in the *same directory* on your computer as this file.
...
Remember that you run chunks using the "Run" button, or **Ctrl+Shift+Enter**.  If the output is long (which the above is!) you can click the "x" on the upper right of the results to not show the output anymore.  You can also run code one line at a time with **Ctrl+Enter**.
...
The first command takes the file **StatesCountries.csv** and assigns it to a new R object called `travel`.  The second and command operates similarly.  The function `read_csv()` also gives some output information about how it read in each of the values. You'll notice both of these objects now show up in the **Environment** pane on the upper-right panel of RStudio, and you can double-click on each dataset to view the values within the data. 

You can also view the data from within the notebook, just by typing its name.

```{r}
travels
```

You can see here that `travel` consists of two vectors: `states` and `countries`. 
...
```{r}
histogram(~ states, data = travels, width = 1, main = "Number of states travelled", xlab = "States Travelled")
```
...
```{r}
histogram(~ states + countries, data = travels, width = 1, main = "Number of states or countries travelled", 
